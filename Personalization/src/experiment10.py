...
def test_word_lists(model, best, worst, train_all, test_A, test_B, path, num_iterations=10):
    ...
    for i in range(num_iterations):
        ...
        results[i] = {
            'worst_A_wer': worst_A_wer - init_A_wer, 'worst_A_cer': worst_A_cer - init_A_cer,
            'worst_B_wer': worst_B_wer - init_B_wer, 'worst_B_cer': worst_B_cer - init_B_cer,
            'worst_AB_wer': worst_AB_wer - init_AB_wer, 'worst_AB_cer': worst_AB_cer - init_AB_cer,
            'best_A_wer': best_A_wer - init_A_wer, 'best_A_cer': best_A_cer - init_A_cer,
            'best_B_wer': best_B_wer - init_B_wer, 'best_B_cer': best_B_cer - init_B_cer,
            'best_AB_wer': best_AB_wer - init_AB_wer, 'best_AB_cer': best_AB_cer - init_AB_cer
        }
...
def main():
    ...
    for id in ids:
        ...
        word_list = load_word_list('word_rankings/{}.txt'.format(id))
        worst_samples = word_list[:100]
        best_samples = word_list[-100:]

        assert len(worst_samples) == 100
        assert len(best_samples) == 100
