...
def compute_correlation(log_path, model, train_all, test_A, test_B):
    ...
    print(colored('Test AB: {:.2f}% WER {:.2f}% CER'.format(base_AB_wer, base_AB_cer), 'blue'))
    ...
    for run in range(num_runs):
        ...
        for step in range(total // step_size + 1):
            ...
            train_samples = train_all[step*step_size:(step+1)*step_size]
...
def main():
    ...
    for id in ids:
        ...
        if not os.path.isdir('../logs/experiment1/' + id):
            os.mkdir('../logs/experiment1/' + id)

        try:
            compute_correlation('../logs/experiment1/' + id + '/', model, train_all, test_A, test_B)
        except Exception as e:
            with open('../logs/experiment1/' + id + '/error.txt', 'w') as f:
                f.write(str(e))
